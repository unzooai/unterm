...
| FR-MCP-004 | Implemented, runtime pending | Each launch generates a UUID token. Instance/active/server/token writes now explicitly use mode `0600` on Unix, including atomic replacements and compatibility handoff. The local cross-platform build blockers found by the `82d15857` CI run are repaired, but the Unix permission test is platform-gated and still awaits a native Unix CI/runtime pass. |

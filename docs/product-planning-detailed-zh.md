...
当前进展：`next-core` 已具备启动环境元数据、typed launch policy provenance、future-launch env overlay、profile/proxy launch-context 诊断、scrollback 文本截图、styled scrollback snapshot 与基础 styled PNG 渲染、styled_scrollback_png capability 诊断、OSC 7 cwd、session activity、input/output/paste 指标、exec.run_wait engine-neutral shell detection、session/exec/screen 核心 MCP handler 共享 pane-id resolver、active recording YAML markdown/redaction、server health 聚合 I/O 诊断、逻辑 viewport goto 和 UTF-8 安全粘贴分块基础能力。
...
- `capture.scrollback` 的 next-core 语义明确：已支持 styled cell snapshot 和基础 styled PNG，后续补完整主题 palette、粗体/斜体字体匹配和 renderer parity。
...
- `selftest.run` 能验证 next-core capability 声明、health I/O payload、profile/proxy launch-context redaction、typed launch policy provenance 和逻辑 viewport 滚动能力。
...
- Windows 11 + ConPTY。
- 单窗口、单 tab、单 shell。
- 基础 VT parse / screen model / scrollback。
...
- benchmark harness，包含 input write、echo、output flood、scrollback paging、viewport scroll、paste、dual-agent、screen-read under flood、focus switch、session create、session ready。

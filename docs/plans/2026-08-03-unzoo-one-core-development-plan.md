...
| M1-03 | 开发中 | IPC 面 36 方法（含交互/录制/引擎健康）；`CoreEngineClient` 满足完整 TerminalEngine；`CoreEventStream` 订阅端；TCP_NODELAY + IPC 成本基准（全量 styled 5.2ms/未变化探询 291µs）；UNTERM_STATE_DIR 隔离；真实进程 E2E 含 8 进程竞锁；单元 9 + E2E 2 通过 | MCP server 迁入 Core 进程；GUI 换用门面与事件流 + FrameCache（M1-04，渲染路径约束见架构文档） |

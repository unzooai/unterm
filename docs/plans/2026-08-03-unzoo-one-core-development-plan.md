...
| M4 | **已完成**（库与桥，未上用户面） | 新建 `unterm-brain`：`BrainEvent` 冻结点 + `BrainAdapter` 纯解析器契约（不起进程、不执行工具，有结构性守卫）；Codex/Claude/SDK 三个适配器，三条路事件同构（多重集比对，SDK 分片先装配再出事件、`stop_reason: tool_use` 算暂停不算回合结束）；`runtime.rs` 真进程：自建进程组、SIGINT→宽限→SIGKILL 且宽限按**整组**计（修掉「agent 先死→孙进程被孤儿化却判成功」的真 bug）、prompt 走 stdin 不进 argv、stderr 尾 40 行、快照随读随写；`supervisor.rs` 把运行绑在 step 租约上，靠「崩溃时无需执行任何代码」实现不丢 task；`health.rs` 连续三次失败才降级、中断/触顶不算故障、只记内存；`unterm-services::brain_tools` 是工具请求进网关的唯一桥，未知工具映射到未分类的 `brain.tool` 必须人批，模型的文件工具有独立动词 `brain.read/list/write/fetch`。brain 51 项 + 桥 6 项。`docs/quality/2026-08-17-m4-brain-runtime.md` | ~~Brain 的 CLI/MCP 面~~ 已落地（`agent_session.*`，随 0.68.0）；真实 agent CLI 端到端与**适配器 wire 格式的录像回放回归仍未做** —— 两个 CLI 的输出格式会随版本漂，没有录像就发现不了 |
| M5 | **已完成**（含真机 Unzoo 端到端） | 新建 `unterm-providers`：端点全部读自 provider 自己的广告（Unzoo 的 `config/rest-port`），有结构性守卫禁止源码写死地址；协议协商接受降级但不接受没听过的版本；身份 TOFU 钉住，变了转 `Degraded` 要人处理；能力分 browser/profile/computer 三家，browser 租约打 cookie 被拒（真机验过）；租约带 epoch+seq，重放在执行前被拒且检查与记录同事务；迁移 v3/v4 落 `capability_leases` 与 `provider_calls`，幂等落库（崩溃后重试不会重复点击）、证据只存 sha256（>16KB 不留原文）；离线→`waiting_provider`、取消→真的传到 provider 且撤租约；fake provider 合约套件（offline/cancel/idempotency/evidence/replay）真机跑通同一套。**补上了 M3 留的洞**：审批此前无人能答（挂到过期＝延迟五分钟的拒绝），现在 `approval.list` 谁都能读、`approval.decide` 拒绝一切网络调用只允许进程内（设置页）——审议边界不是安全边界，已在文档写明。新面：11 个 `provider.*` + 2 个 `approval.*` MCP 方法、`unterm-cli provider` 12 个子命令、设置页 Providers 面板（发现/绑定/暂停/诊断/解绑/撤租约/答审批）。新增合约测试「当前发布的每个方法都要有风险分类」（变异验证过）。`docs/quality/2026-08-17-m5-provider-registry.md` | ~~强制意图路由~~ 已落地（M6-05 `routing.rs`，随 0.68.0）；诊断只跑只读那半套（关掉别人的浏览器不是诊断按钮该干的事）；第三方 provider 需自带 families 表 |
| M6 | **已完成** | `workspace_scope`：隔离是集合性质，scope 每次从全表构造（含已归档根）；嵌套在创建时拒绝并指名是哪一个；`path_scope` 补齐大小写（dev+ino 探真实卷，不按平台猜——猜敏感会让 `/work/SECRET` 溜过 `/work/secret` 的 deny）与 `\\?\`/UNC 归一。`artifacts`：内容寻址落盘（大文件不进 SQLite）、同内容存一份行分开记、删除按引用计数、先写 `.partial` 再 rename、配额够不到时报 `still_over` + `held_by_live_work` 而不是假装成功。`audit_store`：`seq` + 前一条哈希 + 自身哈希的链，改一行/删一行都能定位到第一处断裂；链前旧日志不算篡改；补关联 ID（task/run/step/lease/call/approval/grant/workspace + state）。`evidence`：单 Task 证据包（普通文件 + 哈希清单，`verify` 重算），只含本任务、被扫掉的 artifact 记 `present:false`、两次导出 record 哈希一致。`routing`：绕过 provider 的命令直接拒（不走审批——批准 shell 命令是在问错的问题），词边界匹配、裸 CDP 要回环+devtools 两个条件同时成立。真机:两 workspace 互拒、嵌套被拒、7 条真实审计条目链完整且改一条即定位。127 个 MCP 方法 / `scope|artifact|evidence` 三个 CLI 命令 / 迁移 v5。`docs/quality/2026-08-17-m6-workspace-artifact-audit.md` | 脱敏规则仍在调用方；证据包是目录非压缩包；路由只看命令文本，不可绕过的强制需 OS 级隔离；`scope.*` 尚未接进 brain 的文件工具路径（留 M7） |
| M7 | **部分完成**(代码层完成，含唤醒检测与安装勘察；打包/跨平台真机 E2E 未做） | `supervisor`：三角色独立判定，活着≠可用（`Starting` vs `Ready`）、记录在进程没了是 `Stale` 不是 `Absent`（欠一次清理）、无窗口是受支持状态不是故障、MCP 是独立主张（端口没开的 Core 不是 agent 能去的地方）；机器事件写成一张表（睡眠不动/唤醒重探/注销 drain/关机立停/崩溃 reconcile）。`upgrade`：先快照数据→暂存→交换→确认→失败则二进制与数据双双回退（顺序即设计：先确认再交换是在确认别的东西，先迁移再快照是在快照损坏）；回滚本身也快照；快照只拷值得拷的。`diagnostics`：白名单（黑名单会在有人加字段那天泄漏）、任务只出计数不出标题、路径只出形状，另有能失败的 `leaks_in()` 二道检查。真机：无窗口 Core 上 `system status` 三行齐全、诊断包对着真 token 验证无泄漏；发现并修掉 `supervisor.status` 把内部标签枚举直接上线导致 CLI 全打 `?` 的真 bug。133 个 MCP 方法 / `unterm-cli system` 六个子命令。`docs/quality/2026-08-17-m7-supervisor-delivery.md` | **M7-04 单一安装包与冲突处理未做**；**M7-05 的 Windows/macOS 真机装机-升级-回滚 E2E 未跑**（升级/回滚只在逻辑层验证，真机换掉运行中的应用还要过文件锁/公证/Gatekeeper/MSI 三套 staging）；**唤醒已接**（`wake_watch`：两个时钟不一致即睡眠，跨平台、可喂数字测试，已接进 GUI 空闲 tick，醒来重探 provider + 报到期租约），但注销/关机仍需平台挂钩；**安装勘察与卸载计划已做**（冲突判定 + 只描述不删除），接进真实安装器/卸载器未做 |

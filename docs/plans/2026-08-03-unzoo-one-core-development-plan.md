...
| M1-03 | 开发中 | IPC 面扩至 22 个 session 方法（styled/增量帧/scrollback/search/split 等）；`CoreEngineClient` 门面 + `CoreEventStream` 订阅端；8 项测试通过 | MCP server 迁入 Core 进程；GUI 换用门面与事件流（M1-04） |

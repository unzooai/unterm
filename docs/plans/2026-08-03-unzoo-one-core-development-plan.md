...
| M7 | **部分完成**(代码层完成，装机/跨平台 E2E 未做） | `supervisor`：三角色独立判定，活着≠可用（`Starting` vs `Ready`）、记录在进程没了是 `Stale` 不是 `Absent`（欠一次清理）、无窗口是受支持状态不是故障、MCP 是独立主张（端口没开的 Core 不是 agent 能去的地方）；机器事件写成一张表（睡眠不动/唤醒重探/注销 drain/关机立停/崩溃 reconcile）。`upgrade`：先快照数据→暂存→交换→确认→失败则二进制与数据双双回退（顺序即设计：先确认再交换是在确认别的东西，先迁移再快照是在快照损坏）；回滚本身也快照；快照只拷值得拷的。`diagnostics`：白名单（黑名单会在有人加字段那天泄漏）、任务只出计数不出标题、路径只出形状，另有能失败的 `leaks_in()` 二道检查。真机：无窗口 Core 上 `system status` 三行齐全、诊断包对着真 token 验证无泄漏；发现并修掉 `supervisor.status` 把内部标签枚举直接上线导致 CLI 全打 `?` 的真 bug。133 个 MCP 方法 / `unterm-cli system` 六个子命令。`docs/quality/2026-08-17-m7-supervisor-delivery.md` | **M7-04 单一安装包与冲突处理未做**；**M7-05 的 Windows/macOS 真机装机-升级-回滚 E2E 未跑**（升级/回滚只在逻辑层验证，真机换掉运行中的应用还要过文件锁/公证/Gatekeeper/MSI 三套 staging）；M7-02 的 OS 通知挂钩（NSWorkspace / WM_POWERBROADCAST）未接到状态表上 |

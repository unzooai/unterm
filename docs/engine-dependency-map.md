...
- GUI WebGPU draw now has an opt-in `UNTERM_NEXT_CORE_WEBGPU_PANE` branch: truthy/`append` appends the prepared `NextCoreWebGpuPaneFrame` through `WebGpuState::encode_next_core_pane_frame` after the legacy pass, while experimental `replace` skips legacy pane quad ranges and lets next-core draw the pane while legacy chrome/UI remains visible
...
- GUI `NextCoreWebGpuPaneFrame` now carries the prepared WebGPU frame plus replace diagnostics through the pane draw path, so `replace` mode can share CPU frame preparation between fallback gating and final encode instead of preparing the same pane frame twice
...
- GUI `WebGpuState::encode_next_core_pane_frame` now accepts prepared `NextCoreWebGpuPaneFrame` values, reuses their prepared frame for upload/pass setup, and keeps raw buffer-plan encoding out of the pane draw branch

...
- GUI WebGPU draw now has an opt-in `UNTERM_NEXT_CORE_WEBGPU_PANE` branch that appends the next-core buffer plan pass through `WebGpuState::encode_next_core_buffer_plan` after the legacy pass, keeping default rendering unchanged while proving the real encoder path

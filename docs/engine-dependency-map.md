...
- visible screen read
- styled screen read
- next-core cols-aware screen wrapping, DECAWM auto-wrap mode, DECOM origin mode for scroll-region-relative cursor positioning, HT/HTS/TBC tab-stop cursor movement/control, ESC IND/NEL/RI scroll-region movement, CSI save/restore cursor, erase-character handling, and resize truncation
- visible text read

...
- GUI `EngineRenderPreparedPaneFrame` now carries the prepared WebGPU frame through the engine facade, builds its own replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and owns the replace-request/missing-frame diagnostic selection consumed by the WebGPU draw loop
...
- GUI `WebGpuState::encode_next_core_pane_frame` now accepts prepared `EngineRenderPreparedPaneFrame` values and funnels encoding through a pane-frame helper, reusing the prepared frame for upload/pass setup while keeping raw buffer-plan encoding out of the pane draw branch

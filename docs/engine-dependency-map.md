...
- GUI `EngineRenderPreparedPaneFrame` now carries the prepared WebGPU frame through the engine facade, builds its own replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and lets `replace` mode share CPU frame preparation between fallback gating and final encode instead of preparing the same pane frame twice
...
- GUI `WebGpuState::encode_next_core_pane_frame` now accepts prepared `EngineRenderPreparedPaneFrame` values, reuses their prepared frame for upload/pass setup, and keeps raw buffer-plan encoding out of the pane draw branch

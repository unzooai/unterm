...
|---|---:|---|
| Engine-neutral | 40 | `session.list`, `session.get`, `session.status`, `session.create`, `session.split`, `session.focus`, `session.input`, `session.paste`, `session.idle`, `session.cwd`, `session.history`, `session.resize`, `session.destroy`, `session.recording_start`, `session.recording_stop`, `session.recording_status`, `session.recording_attach_trace`, `session.export_markdown`, `screen.read`, `screen.text`, `screen.scrollback_text`, `screen.cursor`, `screen.search`, `screen.detect_errors`, `exec.run`, `exec.send`, `exec.run_wait`, `exec.status`, `exec.cancel`, `signal.send`, `orchestrate.launch`, `orchestrate.broadcast`, `orchestrate.wait`, `workspace.save`, `workspace.restore`, `screen.scroll`, `fleet.launch`, `fleet.retry`, `server.info`, `server.health` |
| Partial | 0 | None |
...
| WezTerm-only | 16 | `agent.status`, `agent.signal`, `cockpit.inbox`, `fleet.clean`, `ghost.debug`, `capture.screen`, `capture.window`, `capture.select`, `capture.clipboard`, `capture.scrollback`, `capture.window_scroll`, `instance.list`, `instance.info`, `instance.set_title`, `instance.focus`, `system.launch_admin` |
...
| `fleet.clean` | WezTerm-only | May close panes and clean worktrees through the legacy fleet cleanup path | Reuse the extracted fleet pane remover, then keep worktree cleanup product-only. |

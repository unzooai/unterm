...
- visible screen read
- styled screen read
- next-core cols-aware screen wrapping, DECCKM application cursor/navigation key mode, application keypad mode tracking, focus event reporting mode, mouse reporting mode tracking, DECAWM auto-wrap mode, DECOM origin mode for scroll-region-relative cursor positioning, DECSCNM reverse-video mode, IRM insert mode, combined mode set/reset handling, HT/VT/FF/CHT/HTS/TBC/CBT tab/newline cursor movement/control, ESC charset/UTF-8 designator consumption, split-safe DCS/APC/PM/SOS control-string consumption, split-safe C1 CSI/OSC/string-control and IND/NEL/RI handling, DECALN alignment-test fill, ESC IND/NEL/RI scroll-region movement, DECSTR/RIS terminal reset, IL/DL line mutation within scroll regions, CSI CNL/CPL/HPA/HPR/VPA/VPR positioning, CSI/DEC private save/restore cursor, erase-line/erase-character styled blank backfill, delete-character right-margin blank backfill, REP repeat-character handling, CSI 3J scrollback clearing, scrollback ring trimming with stable logical viewport, live-tail following after scrolling to bottom, SGR bold/faint/italic/underline-style/underline-color/strikethrough/hidden/overline/blink/vertical-align/inverse styles, semicolon/colon SGR extended-color parsing, and resize truncation
- visible text read

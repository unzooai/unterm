...
- GUI `EngineRenderGlyphAtlasPlan` now converts text-atlas runs into stable glyph cache keys and cell-aligned glyph instances, giving the future texture atlas renderer concrete per-glyph draw input before font rasterization is attached
...
- GUI `EngineRenderGlyphAtlasTextureUpdatePlan` now turns newly inserted atlas keys into texture update regions with deterministic placeholder RGBA bytes, fixing the future `queue.write_texture` ABI before real font rasterization is attached
- GUI `NextCoreGlyphTexture` now owns a dedicated WebGPU texture atlas and uploads validated next-core glyph texture regions with `queue.write_texture`; the remaining step is binding that atlas to a textured glyph render pass

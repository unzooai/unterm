...
- GUI `EngineRenderPreparedPaneFrame` now carries the prepared WebGPU frame plus replace diagnostics through the engine facade and pane draw path, so `replace` mode can share CPU frame preparation between fallback gating and final encode instead of preparing the same pane frame twice

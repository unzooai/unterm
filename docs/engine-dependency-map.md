...
- next-core CSI params module for SGR, colon-color, underline-style, numeric parameter, and rectangle parsing, keeping parser protocol decoding independently testable from screen mutation semantics
- next-core SGR module for applying parsed style/color/underline/blink/vertical-align parameters to cell attributes, keeping style state transitions independently testable from parser and screen mutation flow
- next-core OSC params module for title, OSC 7 cwd, and OSC 8 hyperlink payload decoding, keeping OSC protocol parsing independently testable from screen state mutation
...
- GUI WebGPU draw now has an opt-in `UNTERM_NEXT_CORE_WEBGPU_PANE` branch: truthy/`append` appends a renderer-owned `NextCoreWebGpuPaneDrawFrame` carrying an engine-prepared frame through `WebGpuState::encode_next_core_pane_frame` after the legacy pass, while experimental `replace` skips legacy pane quad ranges and lets next-core draw the pane while legacy chrome/UI remains visible
- GUI WebGPU `replace` mode now gates legacy pane skipping on both a draw-ready next-core buffer batch and a replace-ready prepared frame; repeated-revision/empty/incomplete-preparation frames keep the legacy pane for that frame and expose structured readiness issues instead of producing a blank pane
- `ci/next-core-gui-render.ps1` now verifies and runs the GUI-side next-core render replacement contract tests, covering the facade, render consumer, buffer-plan conversion, replace diagnostics, and consumer-cache resize path without launching a WebGPU window
- GUI WebGPU `replace` readiness is now represented as a structured diagnostic with mode, pane/revision, batch presence/readiness, prepared-frame presence/readiness, and explicit fallback issues, so future pane replacement automation can consume the gate without replaying draw-loop logic
- GUI `EngineRenderPaneReplaceDiagnostics` now lives behind the engine facade, so WebGPU draw and future CI replacement checks can share the same pane replacement readiness contract
- GUI WebGPU `replace` readiness now also requires cached glyph texture/upload diagnostics for text frames; the preflight uses a cloned glyph atlas state so the gate can inspect upload readiness without mutating the real WebGPU cache before drawing
- GUI WebGPU `replace` readiness now verifies prepared-frame and cached-glyph diagnostics match the buffer batch pane/revision before skipping the legacy pane, preventing stale or cross-pane diagnostics from replacing visible content
- GUI `EngineRenderPreparedPaneFrame` now carries only engine-neutral batch/prepared-frame diagnostics through the engine facade, builds its own replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and owns the replace-request/missing-frame diagnostic selection consumed by the WebGPU draw loop; migration-time `LoadedFont` state is kept in the renderer-owned `NextCoreWebGpuPaneDrawFrame`
- GUI `TermWindow::prepare_next_core_webgpu_pane_frame` now combines pane-id render consumption, default-font lookup, and WebGPU pane-frame preparation before the draw loop, keeping the WebGPU draw loop focused on legacy/next-core pass ordering instead of next-core frame assembly
- GUI `EngineRenderPaneReplaceDiagnostics::requested_missing_frame` now represents a requested replace with no prepared frame through the shared engine contract, so draw-loop fallback logging no longer calls a WebGPU preflight helper just to synthesize missing-frame diagnostics
...
- GUI `EngineWgpuPreparedFramePlan` now exposes replace-readiness diagnostics for solid upload, text atlas, and glyph atlas preparation, giving pane replacement tests a CPU-side gate before real WebGPU encoding
...
- GUI `EngineRenderShapedGlyphPlan` fixes the next input ABI for a real GUI shaper and is now built from engine-neutral `EngineRenderShaperGlyph` runs, carrying text, rect, style, foreground, cells, `font_idx`, and `glyph_pos` before entering the shared atlas/cache/upload path; `wezterm_font::GlyphInfo` conversion lives in the WebGPU renderer
- GUI `EngineRenderGlyphAtlasPlan` now converts text-atlas runs into stable glyph cache keys and cell-aligned glyph instances, and glyph keys can carry optional shaped `(font_idx, glyph_pos)` raster identity; the engine exposes only the `EngineRenderGlyphRasterSource` trait, while the migration-time `LoadedFont::rasterize_glyph` adapter now lives in the WebGPU renderer
- GUI WebGPU next-core pane rendering can now shape text-atlas runs with the default `LoadedFont` and feed the shaped glyph atlas through renderer-owned `NextCoreFontGlyphRasterSource`; if font lookup or shaping fails, the existing deterministic placeholder raster path remains available
...
- GUI `EngineRenderCachedGlyphUploadDiagnostics` now lives behind the engine facade, so cached glyph texture/upload readiness can feed future pane replacement gates and CI checks without depending on WebGPU window-private types
...
- GUI `WebGpuState` now owns the next-core solid/textured glyph shader, vertex layout, pipeline creation, and glyph atlas texture binding before appending the textured glyph pass after the solid next-core pass
...
- GUI `EngineWgpuRenderBackend` converts buffer plans into a POD GPU upload ABI while leaving concrete `wgpu` vertex/index buffer creation to the WebGPU renderer, keeping the engine facade on CPU-side render plans
- GUI `EngineWgpuRenderPassPlan` fixes the minimal indexed draw-pass contract for next-core buffers, so the WebGPU renderer can encode submitted frames without inferring revision or skip semantics
- GUI `WebGpuState` owns the next-core shader/pipeline ABI for solid-color background/text/cursor quads, while the engine facade only emits POD vertices and pass plans with viewport-to-clip conversion kept outside `unterm-engine`
...
- GUI `WebGpuState::encode_next_core_upload` now owns next-core `wgpu` buffer creation and render-pass encoding around prepared next-core GPU upload plans without changing the legacy draw loop yet
- GUI `WebGpuState::encode_next_core_pane_frame` now accepts renderer-owned pane draw frames that wrap `EngineRenderPreparedPaneFrame`, funnels encoding through a pane-frame helper, reuses the engine-prepared frame for upload/pass setup, and keeps raw buffer-plan encoding plus GUI font state out of the pane draw branch
...
The counts intentionally include aliases (`session.get` / `session.status`, `exec.send` via `session.input`) because `meta.surface` exposes them as separate public contracts. The current `MCP_METHODS` inventory contains 103 authenticated methods; `auth.login` is the wire-level pre-dispatch handshake and is not part of `MCP_METHODS`. `unterm-cli mcp-stdio tools/list` exposes 102 callable tools because it excludes the introspection method `meta.surface`; both `meta.surface` and `unterm-cli reference --json` report `mcp_method_count`, `mcp_tool_count`, and `mcp_tool_exclusions` so clients do not have to infer that distinction.

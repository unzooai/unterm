...
|---|---:|---|
| Engine-neutral | 43 | `session.list`, `session.get`, `session.status`, `session.create`, `session.split`, `session.focus`, `session.input`, `session.paste`, `session.idle`, `session.cwd`, `session.history`, `session.resize`, `session.destroy`, `session.recording_start`, `session.recording_stop`, `session.recording_status`, `session.recording_attach_trace`, `session.export_markdown`, `screen.read`, `screen.text`, `screen.scrollback_text`, `screen.cursor`, `screen.search`, `screen.detect_errors`, `exec.run`, `exec.send`, `exec.run_wait`, `exec.status`, `exec.cancel`, `signal.send`, `orchestrate.launch`, `orchestrate.broadcast`, `orchestrate.wait`, `workspace.save`, `workspace.restore`, `screen.scroll`, `agent.status`, `cockpit.inbox`, `fleet.launch`, `fleet.retry`, `fleet.clean`, `server.info`, `server.health` |
| Partial | 4 | `agent.signal`, `capture.screen`, `capture.window`, `capture.scrollback` |
...
| `capture.scrollback` | Partial | `CaptureEngine::render_scrollback_png`; WezTerm adapter renders via existing pane renderer, next-core returns explicit unsupported | Implement next-core rendering from the engine screen/scrollback model. |

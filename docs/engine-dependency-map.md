...
- GUI WebGPU `replace` mode now gates legacy pane skipping on a draw-ready next-core buffer batch; repeated-revision/empty batches keep the legacy pane for that frame and expose structured readiness issues instead of producing a blank pane

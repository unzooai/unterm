...
- GUI `EngineRenderBufferPlan` converts backend commands into damage rects plus quad vertex/index buffers and now carries original `RenderTextRun` metadata, giving the future glyph-atlas path access to text/style/rect data instead of anonymous solid quads

...
- GUI `EngineRenderShapedGlyphPlan` fixes the next input ABI for a real GUI shaper: shaped glyphs can provide text, rect, style, foreground, cells, `font_idx`, and `glyph_pos` before entering the shared atlas/cache/upload path
- GUI `EngineRenderGlyphAtlasPlan` now converts text-atlas runs into stable glyph cache keys and cell-aligned glyph instances, and glyph keys can carry optional shaped `(font_idx, glyph_pos)` raster identity before the real shaper/font raster source is attached

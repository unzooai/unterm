...
- GUI `EngineRenderPreparedPaneFrame` now carries the prepared WebGPU frame through the engine facade, builds its own replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and the shared diagnostics contract exposes the legacy-pane replacement/fallback decisions consumed by the WebGPU draw loop

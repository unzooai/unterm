...
- `EngineRenderPreparedPaneFrame` carries the prepared WebGPU frame through the engine facade, builds replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and exposes the legacy-pane replacement/fallback decisions through the shared diagnostics contract consumed by the WebGPU draw loop

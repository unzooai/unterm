...
- WebGPU `replace` mode now skips legacy pane ranges only when the next-core buffer batch is draw-ready; repeated-revision or empty batches keep the legacy pane for that frame and report structured readiness issues instead of risking a blank pane

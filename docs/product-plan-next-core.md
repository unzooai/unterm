...
- `EngineRenderPreparedPaneFrame` carries the prepared WebGPU frame through the engine facade, builds replace diagnostics from the batch/prepared-frame/cached-glyph inputs, and lets `replace` mode share CPU frame preparation between fallback gating and final encode instead of preparing the same pane frame twice

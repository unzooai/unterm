...
- WebGPU `replace` mode now skips legacy pane ranges only when the next-core buffer batch is draw-ready and the prepared frame is replace-ready; repeated-revision, empty, or incomplete-preparation frames keep the legacy pane for that frame and report structured readiness issues instead of risking a blank pane
- structured WebGPU `replace` readiness diagnostics with mode, pane/revision, buffer-batch readiness, prepared-frame readiness, and explicit fallback issues for future pane replacement automation
- `ci/next-core-gui-render.ps1`, a dedicated GUI render replacement contract gate that verifies and runs the next-core facade, render consumer, buffer-plan, replacement diagnostics, and consumer-cache tests before pane replacement work can be trusted
...
- `EngineRenderPreparedPaneFrame` carries only the engine-prepared WebGPU frame and replace diagnostics through the engine facade, while migration-time GUI font state is owned by `NextCoreWebGpuPaneDrawFrame`; the engine frame builds replace diagnostics from the batch/prepared-frame/cached-glyph inputs and owns replace-request/missing-frame diagnostic selection consumed by the WebGPU draw loop
...
- `EngineRenderShapedGlyphPlan`, the next input ABI for a real GUI shaper, now buildable from engine-neutral `EngineRenderShaperGlyph` runs and carrying shaped text, rect, style, foreground, cells, `font_idx`, and `glyph_pos` into the shared atlas/cache/upload path; `wezterm_font::GlyphInfo` conversion is renderer-owned
- `EngineRenderGlyphAtlasPlan` that turns text-atlas runs into stable glyph cache keys and cell-aligned glyph instances, with optional shaped `(font_idx, glyph_pos)` raster identity; the engine exposes only the `EngineRenderGlyphRasterSource` trait, while the migration-time `LoadedFont::rasterize_glyph` adapter is owned by the WebGPU renderer
- GUI WebGPU next-core pane rendering can shape text-atlas runs with the default `LoadedFont` and upload real raster bytes through renderer-owned `NextCoreFontGlyphRasterSource`, while retaining the deterministic placeholder raster fallback when font lookup or shaping fails
...
- GUI textured glyph placement now exposes a CPU-testable pixel/UV quad contract reused by vertex upload, enabling baseline bitmap placement checks without a live WebGPU window
...
- shared `EngineRenderCachedGlyphUploadDiagnostics` behind the engine facade, letting cached glyph texture/upload readiness feed future pane replacement gates and CI checks without depending on WebGPU window-private types

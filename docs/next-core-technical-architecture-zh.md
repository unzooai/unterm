...
- `next_core/runtime/io_facade.rs` 已拆出 screen/input runtime facade，集中 viewport scroll、plain/styled screen reads、render-frame reads、scrollback reads/search/cursor 和 write/paste input 入口；write/paste 已先构造 `RuntimeCommand` 再交给 input executor 同步执行，并用 queue dry-run 测试覆盖 input byte backpressure，让读屏、翻页、渲染快照、命令补全、键入和粘贴这些高频路径先隔离到可替换模块，后续可以把输入排队、输出合并和 dirty frame 调度接到这个边界上
- `next_core/runtime/queue.rs` 已定义纯内存 bounded runtime command queue，按 command 数量和 pending input bytes 做背压，并暴露 pending/rejected 遥测；当前还未接入 facade 执行路径，后续会先用于 input/paste/write 和 render-frame wakeup 调度

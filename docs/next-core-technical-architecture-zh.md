...
- `next_core/runtime/scheduler.rs` 的 render frame、普通读屏和样式读屏已经改为 `submit_and_dispatch_response` 兼容路径：调用方仍同步拿 `RenderFrameSnapshot` / `ScreenSnapshot` / `StyledScreenSnapshot`，但内部已经通过 response sender/receiver、input-first scheduled dispatch 和 typed `RuntimeDispatchResult` 返回，为后续把 render wakeup 与高频 screen read 移到后台 consumer 提前固定了结果通道

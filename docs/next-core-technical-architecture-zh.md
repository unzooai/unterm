...
- `EngineRenderBufferPlan` 已将 backend command 转为 damage rects、quad vertices 和 indices，并保留原始 `RenderTextRun` 的 text/style/rect 元数据；下一步 glyph atlas 可以消费真实文本信息，而不是只能看到匿名纯色 text quad

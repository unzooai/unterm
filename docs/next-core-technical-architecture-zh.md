...
- `ci/next-core-benchmark.ps1` 默认串起 benchmark summary verifier、size budget、Unicode width/emoji contract、GUI facade render replacement contract、WebGPU pane replacement/glyph contract 和 MCP/product boundary contract；新增 next-core 产品能力必须同时满足性能/体积约束、终端文本宽度正确性、真实渲染替换契约和 agent 可见 API 契约

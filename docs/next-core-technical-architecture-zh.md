...
- `ci/next-core-webgpu-render.ps1` 已把 WebGPU pane replacement、cached glyph upload、glyph atlas cache、texture region validation 和 engine render backend contract 收成独立门禁，覆盖 replace 兜底条件、glyph layout parity、跨 paint cache 复用和 texture upload ABI
...
- `ci/next-core-runtime.ps1` 已把 runtime scheduler/queue/pump contract 收成独立门禁，覆盖 input-first 调度、bounded queue backpressure、lifecycle/focus 优先级、viewport scroll 优先级、screen/render read dispatch、response pump completion 和 runtime health telemetry
- `ci/next-core-benchmark.ps1` 默认串起 benchmark summary verifier、size budget、runtime scheduler/queue/pump contract、Unicode width/emoji contract、GUI facade render replacement contract、WebGPU pane replacement/glyph contract 和 MCP/product boundary contract；新增 next-core 产品能力必须同时满足性能/体积约束、热路径调度隔离、终端文本宽度正确性、真实渲染替换契约和 agent 可见 API 契约

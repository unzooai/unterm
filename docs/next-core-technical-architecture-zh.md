...
- WebGPU draw loop 已提供 `UNTERM_NEXT_CORE_WEBGPU_PANE` 实验分支：默认不启用；`1/true/on/append` 在 legacy pass 后追加 next-core buffer plan pass；`replace` 会跳过 legacy surface rendering，由 next-core 清屏并绘制当前帧，用于验证替代路径

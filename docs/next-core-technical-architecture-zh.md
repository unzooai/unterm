...
- `next_core/session_handles.rs` 已开始集中 session registry 的 handle lookup 和常用 current-handle state 读锁入口，并让必需/可选 recording current handle 复用同一 lookup 入口，让 screen/input/recording/shell/scrollback/query 路径不再反复手写全局状态查找，后续可逐步替换为独立 session registry/runtime

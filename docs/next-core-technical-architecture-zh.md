...
- `next_core/runtime/scheduler.rs` 已把 runtime command queue 接入 input write/paste facade、plain screen read facade、render-frame read facade、scrollback text read facade、screen search facade 和 viewport scroll facade：输入命令先进入 runtime-owned bounded queue，再由 scheduler dequeue 后交给 input executor；读屏、渲染帧读取、历史文本读取、搜索和滚动 mutation 也先经过 command budget，再交给 screen executor。同步行为保持兼容，但大输入、高频渲染读取、普通读屏、历史读取、搜索和翻页滚动已经会经过队列策略和 rejected 遥测；后续只需要把 dequeue 执行替换为独立 consumer，就可以把输入/渲染/翻页/搜索热路径从 UI 同步调用里移走
- `next_core/runtime/screen_executor.rs` 已拆出 screen executor，负责验证 `RuntimeCommand::ReadScreen`、`RuntimeCommand::ReadRenderFrame`、`RuntimeCommand::ReadScrollbackText`、`RuntimeCommand::SearchScreen` 和 `RuntimeCommand::ScrollViewport` 并同步调用 screen dispatch；scheduler 已把 plain read、render-frame read、scrollback text read、screen search 和 viewport scroll 接入 runtime command queue，让高频渲染快照读取、普通读屏、历史读取、搜索和翻页先经过 command budget/backpressure 边界
- `next_core/runtime/io_facade.rs` 已拆出 screen/input runtime facade，集中 viewport scroll、plain/styled screen reads、render-frame reads、scrollback reads/search/cursor 和 write/paste input 入口；write/paste 已先构造 `RuntimeCommand` 再交给 scheduler/input executor 执行，plain screen reads、render-frame reads、scrollback text reads、screen search 和 viewport scroll 也已交给 scheduler/screen executor 执行，并用 queue dry-run 测试覆盖 input byte backpressure，让读屏、翻页、渲染快照、命令补全、键入和粘贴这些高频路径先隔离到可替换模块，后续可以把输入排队、输出合并和 dirty frame 调度接到这个边界上
- `next_core/runtime/queue.rs` 已定义纯内存 bounded runtime command queue，按 command 数量和 pending input bytes 做背压，并暴露 pending/rejected 遥测；当前已接入 input/paste/write、plain screen read、render-frame read、scrollback text read、screen search 和 viewport scroll 的同步兼容路径，后续会继续用于 styled/scrollback range read 与 render wakeup 调度
- `next_core/runtime/recording_facade.rs` 已拆出 recording start/stop/status/trace/export 入口，让录制生命周期继续通过 runtime 边界暴露，但不再膨胀生产 `runtime.rs` 主体
- `next_core/runtime/session_facade.rs` 已拆出 current session operation facade（create/split/id allocation/focus/insert/destroy/resize/session mutation）、只读 session lookup facade 和 session snapshot query facade，让 session creation、session snapshots、session handles 与 runtime current-session 编排先隔离到可替换模块，后续可以把这些入口改造成 runtime command queue 而不继续膨胀生产 `runtime.rs`
- `next_core/runtime/status_facade.rs` 已拆出 shell/output/activity/health status 入口，让 shell query、raw output、session activity 和 aggregate health 通过独立 runtime facade 读取状态，后续可以把 process-tree/health refresh 从显式查询路径进一步异步化

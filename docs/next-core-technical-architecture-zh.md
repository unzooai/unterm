...
- `next_core/test_support.rs` 已拆出测试专用的 state reset、output 注入、dead marker、activity reset 和 viewport attribute probe，并通过 session registry current state/lookup 边界访问测试 session，让 next_core 主体不再承载测试夹具逻辑

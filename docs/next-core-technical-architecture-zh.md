...
- `next_core/runtime/scheduler.rs` 的 render frame 读取已经改为 `submit_and_dispatch_response` 兼容路径：调用方仍同步拿 `RenderFrameSnapshot`，但内部已经通过 response sender/receiver、input-first scheduled dispatch 和 `RuntimeDispatchResult::RenderFrame` 返回，为后续把 render frame wakeup 移到后台 consumer 提前固定了结果通道

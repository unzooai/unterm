...
- `next_core/runtime.rs` 已拆出 `NextCoreRuntime` 容器、current runtime 全局锁入口和 read/write visitor helpers，让 next_core 主体与 session registry 不再直接定义 runtime 存储和锁访问；后续可以在该模块继续下沉 scheduler、event channels、lifecycle ownership 和 registry wiring

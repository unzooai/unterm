...
- `next_core/runtime/scheduler.rs` 已把 runtime command queue 接入 input write/paste facade、plain screen read facade、render-frame read facade 和 viewport scroll facade：输入命令先进入 runtime-owned bounded queue，再由 scheduler dequeue 后交给 input executor；读屏、渲染帧读取和滚动 mutation 也先经过 command budget，再交给 screen executor。同步行为保持兼容，但大输入、高频渲染读取、普通读屏和翻页滚动已经会经过队列策略和 rejected 遥测；后续只需要把 dequeue 执行替换为独立 consumer，就可以把输入/渲染/翻页热路径从 UI 同步调用里移走
- `next_core/runtime/screen_executor.rs` 已拆出 screen executor，负责验证 `RuntimeCommand::ReadScreen`、`RuntimeCommand::ReadRenderFrame` 和 `RuntimeCommand::ScrollViewport` 并同步调用 screen dispatch；scheduler 已把 plain read、render-frame read 和 viewport scroll 接入 runtime command queue，让高频渲染快照读取、普通读屏和翻页先经过 command budget/backpressure 边界
- `next_core/runtime/io_facade.rs` 已拆出 screen/input runtime facade，集中 viewport scroll、plain/styled screen reads、render-frame reads、scrollback reads/search/cursor 和 write/paste input 入口；write/paste 已先构造 `RuntimeCommand` 再交给 scheduler/input executor 执行，plain screen reads、render-frame reads 和 viewport scroll 也已交给 scheduler/screen executor 执行，并用 queue dry-run 测试覆盖 input byte backpressure，让读屏、翻页、渲染快照、命令补全、键入和粘贴这些高频路径先隔离到可替换模块，后续可以把输入排队、输出合并和 dirty frame 调度接到这个边界上
- `next_core/runtime/queue.rs` 已定义纯内存 bounded runtime command queue，按 command 数量和 pending input bytes 做背压，并暴露 pending/rejected 遥测；当前已接入 input/paste/write、plain screen read、render-frame read 和 viewport scroll 的同步兼容路径，后续会继续用于 styled/scrollback/search read 与 render wakeup 调度

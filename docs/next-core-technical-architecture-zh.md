...
- `next_core/session_registry.rs` 已开始集中 session id 分配、session 计数/遍历、active session 切换、session 插入、只读/可变 session lookup、destroy 记账和 current 全局 state lock 入口，让 SessionEngine 后续可替换为独立 registry/runtime 边界

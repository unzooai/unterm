...
- `next_core/runtime/consumer.rs` 已拆出 runtime command 的同步 consumer 边界，集中负责按 command 自身的 lifecycle/input/render/screen/background lane 投递 bounded queue、处理 rejected/lost-command 错误和同步 dequeue；当前仍保持兼容的同步执行，但 scheduler 不再直接操作 queue 或手写 lane，后续可以把 `consume_sync` 替换为独立后台 consumer 或 event loop

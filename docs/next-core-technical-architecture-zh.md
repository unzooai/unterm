...
- `next_core/runtime.rs` 已拆出 `NextCoreRuntime` 容器、current runtime 全局锁入口、read/write visitor helpers、current session operation facade（id allocation/focus/insert/destroy/resize/session mutation）、只读 session lookup facade、session snapshot query facade、shell/output query facade、health/activity query facade 和 recording query facade，让 next_core 主体、session creation、session snapshots、session handles、session activity、session queries、health snapshot、recording lifecycle 与 session registry 不再直接定义 runtime 存储、锁访问或 current-session 编排；后续可以在该模块继续下沉 scheduler、event channels、lifecycle ownership 和 registry wiring

...
- `next_core/runtime/consumer.rs` 已拆出 runtime command 的同步 consumer 边界，集中负责按 command 自身的 lifecycle/input/render/screen/background lane 投递 bounded queue、处理 rejected/lost-command 错误和按 lane dequeue；当前仍保持兼容的同步执行，但 input 同步消费已经不会被已有 screen backlog FIFO 阻塞，并且已经具备 `dispatch_next_scheduled` 这种按策略取下一条并派发执行的后台 consumer 前置入口；scheduler 不再直接操作 queue 或手写 lane，后续可以把 `consume_sync` 替换为独立后台 consumer 或 event loop
- `next_core/runtime/dispatch.rs` 已拆出 runtime command dispatch 边界，用统一 `RuntimeDispatchResult` 表达 input、screen mutation、screen read、render frame、scrollback/search/cursor 等执行结果；当前 lifecycle/recording/status 仍等待 response-channel plumbing，后续后台 consumer 可以先复用这个 dispatch 层，再逐步补齐会话生命周期和状态查询
...
- `next_core/runtime/scheduler.rs` 已把 runtime command queue 接入 input write/paste facade 和全部 screen io facade：输入命令先进入 runtime-owned bounded queue，再由 scheduler dequeue 后交给 input executor；普通/样式读屏、渲染帧读取、可见文本、行范围、scrollback、样式 scrollback、搜索、cursor 和滚动 mutation 也先经过 command budget，再交给 screen executor。同步行为保持兼容，但大输入、高频渲染读取、读屏、历史读取、搜索、cursor 和翻页滚动已经会经过队列策略和 rejected 遥测；后续只需要把 dequeue 执行替换为独立 consumer，就可以把输入/渲染/翻页/搜索热路径从 UI 同步调用里移走
- `next_core/runtime/scheduler.rs` 的全部 screen read 入口已经改为 `submit_and_dispatch_response` 兼容路径：调用方仍同步拿 render frame、普通/样式 screen、visible text、line range、scrollback、styled scrollback、search 和 cursor 结果，但内部已经通过 response sender/receiver、input-first scheduled dispatch 和 typed `RuntimeDispatchResult` 返回，为后续把 render wakeup、高频 screen read、翻页读取、搜索和 cursor 查询移到后台 consumer 提前固定了结果通道

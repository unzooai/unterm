...
- `next_core/session_registry.rs` 已开始集中 session store newtype 及其 len/iter/lookup/push/remove 方法、session id 分配、active session 切换、destroy 记账、current 读/写 state 边界和 current session mutation 边界，让 SessionEngine 后续可替换为独立 registry/runtime 边界

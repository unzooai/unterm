...
- `next_core/runtime/scheduler.rs` 的 input write/paste 和全部 screen read 入口已经改为 `submit_and_dispatch_response` 兼容路径：调用方仍同步拿输入完成、render frame、普通/样式 screen、visible text、line range、scrollback、styled scrollback、search 和 cursor 结果，但内部已经通过 response sender/receiver、input-first scheduled dispatch 和 typed `RuntimeDispatchResult` 返回，为后续把输入、render wakeup、高频 screen read、翻页读取、搜索和 cursor 查询移到后台 consumer 提前固定了结果通道

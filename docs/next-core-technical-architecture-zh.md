...
- `next_core/runtime/scheduler.rs` 已把 runtime command queue 接入 input write/paste facade 和全部 screen io facade：输入命令、普通/样式读屏、渲染帧读取、可见文本、行范围、scrollback、样式 scrollback、搜索、cursor 和滚动 mutation 都先经过 command budget 与 rejected 遥测；同步行为保持兼容，但大输入、高频渲染读取、读屏、历史读取、搜索、cursor 和翻页滚动已经有统一队列边界
- `next_core/runtime/scheduler.rs` 的 input write/paste、viewport scroll mutation 和全部 screen read 入口已经改为 `submit_and_dispatch_response` 兼容路径：调用方仍同步拿输入完成、翻页完成、render frame、普通/样式 screen、visible text、line range、scrollback、styled scrollback、search 和 cursor 结果，但内部已经通过 response sender/receiver、input-first scheduled dispatch 和 typed `RuntimeDispatchResult` 返回，为后续把输入、render wakeup、高频 screen read、翻页滚动、搜索和 cursor 查询移到后台 consumer 提前固定了结果通道

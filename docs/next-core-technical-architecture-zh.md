...
- `next_core/runtime/pump.rs` 已拆出兼容同步 runtime pump 边界，负责按 `RuntimeSchedulePolicy` 取下一条命令、执行 dispatch、完成 attached response，并提供 `drain_until_response`；这把“提交命令”和“泵调度直到响应完成”分开，为后续后台线程/event loop 替换同步 pump 提前固定了接口

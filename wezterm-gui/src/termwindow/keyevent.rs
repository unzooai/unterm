...
impl super::TermWindow {
    ...
    /// Handle keyboard input when AI chat is focused.
    /// Returns true if the key was consumed.
    fn handle_ai_chat_key(
        &mut self,
        key: &KeyEvent,
        pane: &Arc<dyn mux::pane::Pane>,
        context: &dyn WindowOps,
    ) -> bool {
        use crate::ai::models::{ai_state, ChatMessage, ChatRole, InsightCard, InsightType};

        let state = ai_state();

        // Check for modifier-only keys (Ctrl, Alt, Super) — let those pass through
        let dominated_mods = key.modifiers & (Modifiers::CTRL | Modifiers::ALT | Modifiers::SUPER);

        match &key.key {
            // Escape — close AI panel entirely
            KeyCode::Char('\x1b') => {
                state.set_chat_focused(false);
                state.set_panel_visible(false);
                // Re-apply dimensions so terminal reclaims the panel area
                if let Some(window) = self.window.clone() {
            ...
            KeyCode::Char('\r') => {
                // Submit chat message
                let input = state.chat_input_take();
                ...
                // Spawn AI chat in background
                let pane_id = pane.pane_id();
                let screen_context = self.get_chat_screen_context(pane);
                std::thread::spawn(move || {
                    let config = config::configuration();
                    let model_name = ai_state().active_model();
                    let provider = crate::ai::models::ModelProvider::from_name(&model_name);
                    let (api_key, model_id) = match &provider {
                        crate::ai::models::ModelProvider::Claude => (
                            config.ai_claude_api_key.clone(),
                            config.ai_claude_model.clone(),
                        ),
                        crate::ai::models::ModelProvider::OpenAI => (
                            config.ai_openai_api_key.clone(),
                            config.ai_openai_model.clone(),
                        ),
                        crate::ai::models::ModelProvider::Gemini => (
                            config.ai_gemini_api_key.clone(),
                            config.ai_gemini_model.clone(),
                        ),
                        crate::ai::models::ModelProvider::Custom => {
                            return;
                        }
                    };

                    let shell_type = if let Some(mux) = mux::Mux::try_get() {
                        if let Some(p) = mux.get_pane(pane_id) {
            ...
            KeyCode::Char('\x08') => {
                state.chat_input_pop();
                context.invalidate();
                return true;
            }
            // Regular character input — allow if no Ctrl/Alt/Super modifiers
            KeyCode::Char(c) if dominated_mods.is_empty() => {
                state.chat_input_push(*c);
                context.invalidate();
                return true;
            }
            _ => {
                // Let modifier keys (Ctrl+Shift+I etc.) pass through
                return false;
            }
        }
    }

    fn get_chat_screen_context(&self, pane: &Arc<dyn mux::pane::Pane>) -> String {
    ...
    pub fn key_event_impl(&mut self, window_key: KeyEvent, context: &dyn WindowOps) {
        ...
        // AI Chat input handling — intercept keys when chat is focused
        if window_key.key_is_down && crate::ai::models::ai_state().chat_focused() {
            log::trace!(
                "AI chat key: {:?} modifiers={:?}",
                window_key.key,
                window_key.modifiers
            );
            let handled = self.handle_ai_chat_key(&window_key, &pane, context);
            if handled {
                return;
            }
        }

        // Ghost text: Tab with no modifiers accepts the suggestion
        if window_key.key_is_down
            && window_key.modifiers.is_empty()
            && matches!(window_key.key, KeyCode::Char('\t'))
        {
            if let Some(ghost) = crate::ghost_text::ghost_text_state().take() {
                if ghost.pane_id == pane.pane_id() {
                    if let Err(err) = pane.writer().write_all(ghost.text.as_bytes()) {
                        log::error!("Failed to write ghost text: {err}");
                    }
                    context.invalidate();
                    return;
                } else {
                    // Put it back if it's for a different pane
                    crate::ghost_text::ghost_text_state().set(ghost);
                }
            }
        }

        // Any non-Tab keypress clears ghost text and schedules new suggestion
        if window_key.key_is_down && !matches!(window_key.key, KeyCode::Char('\t')) {
            crate::ghost_text::ghost_text_state().clear();
            // Schedule auto ghost text on printable character input (no modifiers)
            if window_key.modifiers.is_empty() {
                if let KeyCode::Char(c) = &window_key.key {
                    if !c.is_control() {
                        crate::ghost_text::schedule_auto_suggest(pane.pane_id());
                    }
                }
            }
        }

...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let mut order: Vec<&'static str> = vec!["Unterm", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    ...
                    } else if cmd.menubar[0] == "Unterm" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Unterm {}", config::wezterm_version()),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        // Unterm: Ctrl+C copies if selected, otherwise sends ^C (like Windows Terminal)
        CopyToOrSendKey(_) => CommandDef {
            brief: "Copy or send key".into(),
            doc: "Copies selection if any, otherwise sends the key to the terminal".into(),
            keys: vec![
                (Modifiers::CTRL, "c".into()),
            ],
            args: &[ArgType::ActivePane],
            menubar: &["Edit"],
            icon: Some("md_content_copy"),
        ...
        QuitApplication => CommandDef {
            brief: "Quit Unterm".into(),
            doc: "Quits Unterm".into(),
        ...
        ShowLauncherArgs(_) | ShowLauncher => CommandDef {
            brief: "New Tab (Shell Selector)".into(),
            doc: "Shows the launcher to select a shell for a new tab".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "Space".into())],
            args: &[ArgType::ActiveWindow],
            menubar: &["Shell"],
            icon: Some("cod_terminal"),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://github.com/user/unterm" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the Unterm documentation".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/user/unterm/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit Unterm GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/user/unterm/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit Unterm GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
        ...
        DismissSuggestion => CommandDef {
            ...
            doc: "Dismiss the oldest pending MCP `session.suggest` proposal on the active pane; also blocks a pending MCP input confirmation if one is showing".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Edit"],
            icon: None,
        },
        McpConfirmAllow => CommandDef {
            brief: "Allow pending MCP input".into(),
            doc: "Allow the oldest pending MCP confirmation banner (one-shot)".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Edit"],
            icon: None,
        },
        McpConfirmAlwaysAllow => CommandDef {
            brief: "Always allow this MCP agent".into(),
            doc: "Allow the pending MCP confirmation AND skip the banner for any further calls by the same agent during this session".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Edit"],
            icon: None,
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ResetTerminal,
        // ----------------- Edit
        // Unterm: Ctrl+C = copy if selected, else send ^C (Windows Terminal behavior)
        CopyToOrSendKey(ClipboardCopyDestination::Clipboard),
        #[cfg(not(target_os = "macos"))]
        ...
        ShowLauncherArgs(LauncherActionArgs {
            ...
            title: Some("Select Shell / Tab".to_string()),

...
#[derive(Debug, Parser)]
#[command(
    about = "Unterm - Native Super Terminal\nhttps://github.com/user/unterm",
...
async fn async_run_terminal_gui(
    ...
    let mut spawn_command = match (spawn_command, &opts.domain) {
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    if let Some(pos) = opts.position.as_ref() {
        ...
        // No explicit position �?try to restore from last session
        if let Some(saved) = crate::session_state::load_session_state() {
            let pos = config::GuiPosition {
                x: config::Dimension::Pixels(saved.x as f32),
                y: config::Dimension::Pixels(saved.y as f32),
                origin: config::GeometryOrigin::ScreenCoordinateSystem,
            };
            set_window_position(pos);
            set_saved_dimensions(saved.width, saved.height);
            log::info!(
                "Restored window position from last session: ({}, {}), size: {}x{}",
                saved.x,
                saved.y,
                saved.width,
                saved.height,
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("Unterm Error", &err);
    std::process::exit(1);
}
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by unterm because custom_block_glyphs=true: {:?}",

...
Both listeners are loopback-only. The MCP connection must call `auth.login` before any of the 103 authenticated methods. The Web application can bootstrap locally, but all `/api/*` routes require the same bearer token.
...
| `unterm-mcp` | Authenticated JSON-RPC listener and dispatch for the 103 public methods. |

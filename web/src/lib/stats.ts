...
export interface Stats {
  ...
  /** Latest tag, e.g. "v0.7.0". Used for download links so we don't have
   *  to update the hero CTA every time we cut a release. */
  release: string;
}

// Fallback used when the build-time fetch fails entirely. We give the
// release a sane default (v0.7.0) because the hero CTA must point
...
const FALLBACK: Stats = { stars: null, downloads: null, release: "v0.7.0" };

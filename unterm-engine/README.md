...
The render list itself comes from next-core's tab registry: it decides which panes exist and where they sit, and the mux is left holding the pane handles, looked up by id. It falls back to the mux's list whenever the registry cannot account for exactly the same panes -- rendering a subset would make a pane vanish, which is far worse than deferring the swap for a frame. The registry is driven by events. `MuxNotification::PaneSplit` carries what `PaneAdded` cannot -- which pane was divided and along which axis -- so the registry records a split as it happens instead of rebuilding its tree from the mux's geometry afterwards. Adoption remains as the bootstrap for a tab it has never seen, and as repair for anything that still changes structure without saying so; both are logged, so the residue is measurable rather than asserted. A GUI run with two nested splits logs 1 bootstrap, 0 repairs, and 0 geometry differences, down from 2 repairs before splits were reported.

The remaining bootstrap is the window's first tab. Its first paint happens before any registration point tried so far, so the registry learns it by adopting. That tab has one pane and therefore no split structure to infer, which is why this is left as it is rather than reordering window construction around it. Tabs opened later are registered from `TabAddedToWindow`.

Close is wired through the registry on the GUI close paths, being the one structural event that arrives intact -- removing a leaf needs no knowledge of how the tree was built. An MCP `session.destroy` still goes through the engine rather than those paths and is picked up by repair.

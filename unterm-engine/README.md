...
The render list itself comes from next-core's tab registry: it decides which panes exist and where they sit, and the mux is left holding the pane handles, looked up by id. It falls back to the mux's list whenever the registry cannot account for exactly the same panes -- rendering a subset would make a pane vanish, which is far worse than deferring the swap for a frame. The registry is meant to be driven by events. Adoption is the bootstrap for a tab it has never seen and the repair when something changed the structure without telling it, and each is logged so the remaining dependence on the mux is measurable rather than asserted. A GUI run with two nested splits logs 1 bootstrap and 2 repairs -- one per split, because `MuxNotification::PaneAdded` carries no split structure, so a tree cannot be rebuilt from it. Close is the one structural event that arrives intact (removing a leaf needs no knowledge of how the tree was built) and is wired through the registry on the GUI close paths, though an MCP `session.destroy` still bypasses them and is picked up by repair instead.

Retiring the mux as the origin of structure needs splits reported structurally -- which pane was split, on which axis. Until then the repair count is exactly the amount of structure the mux still originates.

...
On Windows, the command above should print `next-core-probe`. A blank snapshot means the standalone ConPTY smoke path has regressed before the GUI is involved. The in-memory screen model is cols-aware: printable text wraps at the configured width, DECAWM `CSI ? 7 h/l` controls automatic wrapping at the right edge, DECOM `CSI ? 6 h/l` makes cursor positioning relative to scroll regions, IRM `CSI 4 h/l` inserts printable cells before overwriting, combined mode set/reset parameters are applied together, HT/CHT moves the cursor to the next tab stops without forcing a line wrap, HTS/TBC set and clear custom tab stops, CBT reverse-tabs to prior tab stops, ESC IND/NEL/RI/RIS and IL/DL line mutation honor scroll regions, CSI CNL/CPL/HPA/HPR/VPA/VPR plus CSI/DEC private save/restore cursor, erase-line/erase-character styled blank backfill, delete-character right-margin backfill, REP repeat-character, `CSI 3 J` scrollback-clear, and semicolon/colon SGR extended-color sequences are handled, wide cells wrap before the edge, and resize clamps existing rows to the new column count.

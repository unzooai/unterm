...
`SessionEngine::activity()` is based on `next-core`'s own PTY liveness and recent input/output timestamps. A session is reported as running shortly after input or output and idle after a quiet period. The activity snapshot also includes input, output, paste, screen-read, and viewport-scroll counters so agents can diagnose slow typing, completion, auth-code paste, heavy agent output, and PageUp/PageDown stalls. `HealthEngine::health()` exposes the same class of counters as an aggregate `io` summary across live `next-core` sessions for server-level diagnostics. The foreground process name is still the launch shell until child-process tracking is added.

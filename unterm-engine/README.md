...
The render list itself comes from next-core's tab registry: it decides which panes exist and where they sit, and the mux is left holding the pane handles, looked up by id. It falls back to the mux's list whenever the registry cannot account for exactly the same panes -- rendering a subset would make a pane vanish, which is far worse than deferring the swap for a frame. The registry is still mirrored from the mux each frame rather than fed by events, so the mux remains the origin of the structure even though it no longer drives rendering.

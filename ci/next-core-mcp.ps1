...
try {
    ...
    if (-not @($run | Where-Object { $_ -match "test result: ok\..*40 passed" })) {
        throw "next-core MCP contract test run did not report 40 passed tests"
    }

    Write-Host "next-core MCP contract tests ok: required=$($RequiredTests.Count) module_tests=40"
} finally {
    Pop-Location
}

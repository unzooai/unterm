...
try {
    ...
    if (-not @($run | Where-Object { $_ -match "test result: ok\..*36 passed" })) {
        throw "next-core MCP contract test run did not report 36 passed tests"
    }

    Write-Host "next-core MCP contract tests ok: required=$($RequiredTests.Count) module_tests=36"
} finally {
    Pop-Location
}

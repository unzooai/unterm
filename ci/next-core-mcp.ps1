...
try {
    ...
    if (-not @($run | Where-Object { $_ -match "test result: ok\..*39 passed" })) {
        throw "next-core MCP contract test run did not report 39 passed tests"
    }

    Write-Host "next-core MCP contract tests ok: required=$($RequiredTests.Count) module_tests=39"
} finally {
    Pop-Location
}

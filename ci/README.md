...
The default mode verifies `docs\next-core-benchmark-summary.json` with `unterm-engine\verify-next-core-benchmark.ps1`. The full mode runs `unterm-engine\bench-next-core.ps1`, writes the Markdown/JSON benchmark artifacts, then verifies the JSON summary.

...
pub fn set_wezterm_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            config::env_names::set_var("EXECUTABLE_DIR", dir);
        }
        config::env_names::set_var("EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        if let Some(dir) = config::env_names::var_os("EXECUTABLE_DIR") {
        ...
        fn clean_wezterm_config_env() {
            config::env_names::remove_var("CONFIG_FILE");
            config::env_names::remove_var("CONFIG_DIR");
        }

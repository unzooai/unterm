...
// Unterm: 灰色 tab bar 配色（与纯黑终端区域对比）
fn default_background() -> RgbaColor {
    RgbColor::new_8bpc(0x2d, 0x2d, 0x2d).into() // 深灰 tab bar 背景
}

fn default_inactive_tab_edge() -> RgbaColor {
    RgbColor::new_8bpc(0x3a, 0x3a, 0x3a).into()
}

fn default_inactive_tab_edge_hover() -> RgbaColor {
    RgbColor::new_8bpc(0x4a, 0x4a, 0x4a).into()
}

fn default_inactive_tab() -> TabBarColor {
    TabBarColor {
        bg_color: (0x2d, 0x2d, 0x2d).into(), // 深灰
        fg_color: (0x99, 0x99, 0x99).into(), // 浅灰文字
        ..TabBarColor::default()
    }
}
fn default_inactive_tab_hover() -> TabBarColor {
    TabBarColor {
        bg_color: (0x3f, 0x3f, 0x3f).into(), // hover 时稍亮
        fg_color: (0xe0, 0xe0, 0xe0).into(), // 亮白文字
        italic: false,
        ..TabBarColor::default()
    }
}
fn default_active_tab() -> TabBarColor {
    TabBarColor {
        bg_color: (0x0c, 0x0c, 0x0c).into(), // 纯黑 = 和终端背景融合
        fg_color: (0xff, 0xff, 0xff).into(), // 纯白
        ..TabBarColor::default()
    }
}
...
fn default_active_titlebar_bg() -> RgbaColor {
    RgbColor::new_8bpc(0x2c, 0x2c, 0x2c).into()
}

fn default_inactive_titlebar_fg() -> RgbaColor {
    RgbColor::new_8bpc(0xa6, 0xa6, 0xa6).into()
}

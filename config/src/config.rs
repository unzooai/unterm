...
pub struct Config {
    ...
    // Unterm: fancy tab bar 有关闭按钮和+按钮，类�?Windows Terminal
    #[dynamic(default = "default_true")]
    ...
    // Default ON. It was disabled because an inner split pane's scrollbar sat
    // at the split column (window centre) and drew a band down the middle. Now
    // fixed two ways: the renderer only draws the bar for a pane at the window's
    // right edge (never at a split column — see render/pane.rs), and every
    // Unterm scheme defines a theme-matched `scrollbar_thumb`, so the thumb is a
    // subtle visible gutter rather than an invisible-or-black strip.
    #[dynamic(default = "default_true")]
    ...
    /// Only works on Windows. Disabled by default to match the opaque
    /// Windows Terminal / PowerShell content surface.
    #[dynamic(default = "default_system_backdrop")]
    ...
    /// performance.  Unterm defaults to 1.0 to match Windows Terminal's
    /// opaque PowerShell profile.
    #[dynamic(default = "default_window_opacity")]
...
fn default_color_scheme() -> Option<String> {
    ...
    // 让「选主题」成为可靠的全局配色来源;无文件时回退到中性高对比
    // Unterm Dark —— 纯中性近黑底 + 纯白字(~14.6:1 对比),复刻 Warp 的
    // 沉稳清晰,左侧 tab 栏在它下面观感最佳。
    // 用户在 unterm.lua 显式设置 color_scheme 仍会覆盖此默认值。
    read_theme_json_scheme().or_else(|| Some("Unterm Dark".to_string()))
}
...
fn default_default_prog() -> Option<Vec<String>> {
    #[cfg(windows)]
    {
        ...
        if std::path::Path::new(pwsh).exists() {
            return Some(vec![pwsh.to_string(), "-NoLogo".to_string()]);
        }
        return Some(vec!["powershell.exe".to_string(), "-NoLogo".to_string()]);
    }
...
fn default_launch_menu() -> Vec<SpawnCommand> {
    ...
    #[cfg(windows)]
    {
        ...
        if std::path::Path::new("C:\\Program Files\\PowerShell\\7\\pwsh.exe").exists() {
            menu.push(SpawnCommand {
                label: Some("PowerShell 7".to_string()),
                args: Some(vec![
        ...
        menu.push(SpawnCommand {
            label: Some("Windows PowerShell".to_string()),
        ...
        // CMD
        menu.push(SpawnCommand {
            label: Some("Command Prompt (CMD)".to_string()),
        ...
        for path in &git_bash_paths {
            if std::path::Path::new(path).exists() {
                menu.push(SpawnCommand {
                    label: Some("Git Bash".to_string()),
                    args: Some(vec![path.to_string(), "--login".to_string()]),
                    ..SpawnCommand::default()
                });
                break;
            }
        ...
        if std::path::Path::new("C:\\Windows\\System32\\wsl.exe").exists() {
            menu.push(SpawnCommand {
                label: Some("WSL (Default)".to_string()),
                args: Some(vec!["wsl.exe".to_string()]),
                ..SpawnCommand::default()
            });
        }
        ...
        if std::path::Path::new("C:\\msys64\\usr\\bin\\bash.exe").exists() {
            menu.push(SpawnCommand {
                label: Some("MSYS2 Bash".to_string()),
                args: Some(vec![
        ...
        for path in &nushell_paths {
            if std::path::Path::new(path).exists() {
                menu.push(SpawnCommand {
                    label: Some("Nushell".to_string()),
...
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    if let Some(runtime) = dirs_next::cache_dir() {
        return Ok(runtime.join("unterm"));
    }

    Ok(crate::HOME_DIR.join(".local/share/unterm"))
}
...
fn default_system_backdrop() -> SystemBackdrop {
    SystemBackdrop::Disable
}
...
pub enum ImePreeditRendering {
    /// IME preedit is rendered by Unterm itself
    #[default]
    Builtin,
